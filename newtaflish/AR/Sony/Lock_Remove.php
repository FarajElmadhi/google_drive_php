...
$stmt = $connect->prepare(" SELECT * FROM sonydevice ORDER BY Model ASC");
$stmt->execute();
$roms = $stmt->fetchAll();
...
<meta name="description" content=" تحميل ملفات ازالة القفل بدون تحميل روم كامل لهواتف سوني أحجام الملفات لاتتجاوز 1 ميغا بايت">
<meta name="keywords" content="ملفات ازالة قفل سوني,ملف حذف نمط سوني,تجاوز نمط سوني,تجاوز قفل سوني,فومات سوني,الدخول الى الريكفري اكسبيريا,lock remove ftf sony xperia,bypasse lock screen sony xperia,download rom xperia,download recovery sony xperia,download remove pattern sony xperia">
<title>ملفات ازالة قفل سوني </title>
...
></span></a>
            <ul class=" dropdown-menu text-right" >
              ...
              <li><a class="text-right" href="../../EN/Sony/Lock_Remove.php">English</a></li>
            </ul>
          </li>
    ...
    <div class="row">
      <div class="col-xs-12 visible-xs">
        <div class="all-device-mobile">
          <h4 class="wow flash" data-wow-duration="4s" data-wow-iteration="100" >ملفات ازالة القفل لهواتف سوني - ضبط المصنع </h4>
        </div>
        <?php  foreach($roms as $row){ ?>
          <div class="mobile-table btn-block text-center "> <a href="View_Lock_Remove.php?Model=<?php echo $row['Model']; ?>">   <div class="file-type ">ملف ازالة القفل<p><pr> <span class="glyphicon glyphicon glyphicon-eye-open"></span> <?php echo $row['view']; ?></p></div> <?php echo $row['Model_Name']; ?> - <?php echo $row['Model']; ?> </a></div>
          <?php }   ?>
      </div>
    </div>
    <div>
              ...
              <tbody>
                <?php  foreach($roms as $row){ ?>
                  <tr>
                    ...
                    <td><h4><?php echo $row['Companny']; ?>  ( ملف ازالة القفل ) </h4></td>
                    <td><h4><?php echo $row['Model_Name']; ?></h4></td>
                    <td><h4><?php echo $row['Model']; ?></h4></td>
                    <td><a class="link-main" href="View_Lock_Remove.php?Model=<?php echo $row['Model']; ?>" > عرض <span class="fa fa-chevron-right"> </span></a></td>
                  </tr>
                  <?php }    ?>
              </tbody>
            </table>
