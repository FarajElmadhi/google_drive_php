...
<meta name="description" content="تحميل ملف ازالة القفل - ازالة النمط- لهاتف سوني <?php echo $row['Model_Name']; ?> موديل <?php echo $row['Model']; ?> جميع الاصدارات">
<meta name="keywords" content="ملفات ازالة قفل سوني <?php echo $row['Model']; ?>,ملف حذف نمط سوني <?php echo $row['Model']; ?>,تجاوز نمط سوني <?php echo $row['Model']; ?>,تجاوز قفل سوني,فومات سوني <?php echo $row['Model']; ?> ,الدخول الى الريكفري اكسبيريا <?php echo $row['Model']; ?>,lock remove ftf sony xperia <?php echo $row['Model']; ?>,bypasse lock screen sony xperia <?php echo $row['Model']; ?>,download rom xperia <?php echo $row['Model']; ?>,download recovery sony xperia <?php echo $row['Model']; ?> ,download remove pattern sony xperia <?php echo $row['Model']; ?>,ملفات ازالة قفل سوني <?php echo $row['Model_Name']; ?>,ملف حذف نمط سوني,تجاوز نمط سوني <?php echo $row['Model_Name']; ?>,تجاوز قفل سوني <?php echo $row['Model_Name']; ?>,فومات سوني <?php echo $row['Model_Name']; ?>,الدخول الى الريكفري <?php echo $row['Model_Name']; ?>,lock remove ftf sony  <?php echo $row['Model_Name']; ?>,bypasse lock screen sony xperia <?php echo $row['Model_Name']; ?>,download rom  <?php echo $row['Model_Name']; ?>,download recovery sony <?php echo $row['Model_Name']; ?>,download remove pattern sony <?php echo $row['Model_Name']; ?>">
<title> تحميل ملف ازالة قفل - نمط   <?php echo $row['Model_Name']; ?> - <?php echo $row['Model']; ?> </title>
...
></span></a>
            <ul class=" dropdown-menu text-right" >
              ...
              <li><a class="text-right" href="../../EN/Sony/View_Lock_Remove.php?Model=<?php echo $row['Model']; ?>">English</a></li>
            </ul>
          </li>
      ...
      <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-2">
        <ul class="nav navbar-nav navbar-right">
          ...
          <li class="dropdown" dir="rtl"> <a href="#" class="dropdown-toggle text-right" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">آبــل<span class="caret"></span></a>
           ...
           <li class="dropdown active" dir="rtl"> <a href="#" class="dropdown-toggle text-right" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">سوني<span class="caret"></span></a>
            <ul class="dropdown-menu">
              <li><a class="text-right"  href="Rom.php">رومات سوني الرسمية</a></li>
              <li role="separator" class="divider"></li>
              <li><a class="text-right"  href="Lock_Remove.php">ملفات ضبط المصنع </a></li>
            </ul>
          </li>
    ...
    <div class="row">
      <div class="col-xs-12 visible-xs">
        <div class="all-device-mobile">
          <h4 class="wow flash" data-wow-duration="4s" data-wow-iteration="100" ><?php echo $row['Model_Name']; ?>  (<?php echo $row['Model']; ?>)  ملفات ازالة القفل لهاتف </h4>
        </div>
   <?php  foreach($roms as $row){ ?>

       <div class="mobile-table btn-block text-center "> <a href="Download_Lock_Remove.php?LRid=<?php echo $row['LRid']; ?>"> <div class="file-type ">ملف ازالة القفل<p><pr> <span class="glyphicon glyphicon glyphicon-eye-open"></span> <?php echo $row['view']; ?></p></div> <?php echo $row['Model_Name']; ?>   - <?php echo $row['Besband']; ?>  </a></div>
                          <?php }     ?>
              ...
              <tbody>
                ...
                <?php  foreach($roms as $row){ ?>
                  <tr>
                    ...
                    <td><h4>SONY  ( ملف ازالة القفل -  FTF ) </h4></td>
                    <td><h4><?php echo $row['Model_Name']; ?></h4></td>
                    <td><h4><?php echo $row['Model']; ?></h4></td>
                    <td><h4><?php echo $row['Besband']; ?></h4></td>

                    <td><a class="link-main" href="Download_Lock_Remove.php?LRid=<?php echo $row['LRid']; ?>"> عرض  <span class="fa fa-chevron-right"> </span></a></td>
                  </tr>
                  <?php }   ?>
              </tbody>
            </table>
