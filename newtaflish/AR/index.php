...
$stmt = $connect->prepare(" SELECT * FROM samsung_device ORDER BY Model ASC");
$stmt->execute();
$roms = $stmt->fetchAll();
...
<meta name="description" content="Biggest Site For Download Firmware All Mobile Phone : Samsung , Sony , iPhone , Nokia , And More ">
<meta name="keywords" content="taflish,تفليش,all rom samsung official,all stock rom samsung,all stock firmware samsung,all repaire firmware samsung,all rom samsung 4 files,all root samsung,all cert file samsung, all modem file,all combination files samsung,all combination rom samsung, samsung,firmware samsung,all rom Sony,all firmware sony xperia,firmwar Galaxy,rom galaxy,rom Xperia,firmware xperia,firmware iphone,firmware apple,firmware ipad,firmware ipod,ipsw iphone,ipsw ipad,ipsw ipod,all lock remove ftf files sony xperia,hard reset sony xperia,all rom nokia,all flash file nokia,twrp samsung,cf root samsung,root samsung,recovery samsung,custom recovery samsung,all stock recovery samsung,موقع تفليش,,">
...
></span></a>
            <ul class=" dropdown-menu" >
              ...
              <li><a class="text-right" href="../../">English</a></li>
            </ul>
          </li>
      ...
      <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-2">
        <ul class="nav navbar-nav navbar-right">
          ...
          <li class="dropdown" dir="rtl"> <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">آبــل<span class="caret"></span></a>
           ...
           <li class="dropdown active " dir="rtl"> <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">سامسونج<span class="caret"></span></a>
            <ul class="dropdown-menu">
    ...
    <div class="row">
      <div class="col-xs-12 visible-xs">
        <div class="all-device-mobile">
          <h4 class="wow flash" data-wow-duration="4s" data-wow-iteration="100" >الرومات الرسمية لهواتف سامسونج </h4>
        </div>
   <?php  foreach($roms as $row){ ?>

        <div class="mobile-table btn-block text-center "> <a href="Samsung/View_Rom.php?Model=<?php echo $row['Model']; ?>"><div class="file-type "> روم رسمي<p><pr> <span class="glyphicon glyphicon glyphicon-eye-open"></span> <?php echo $row['view']; ?></p></div> 
         <?php echo $row['Model_Name']; ?> - <?php echo $row['Model']; ?>  </a></div>
                          <?php }   ?>
              ...
              <tbody>
                ...
                <?php  foreach($roms as $row){ ?>
                  <tr>
                    ...
                    <td><h4><?php echo $row['Company']; ?>  ( روم رسمي ) </h4></td>
                    <td><h4><?php echo $row['Model_Name']; ?></h4></td>
                    <td><h4><?php echo $row['Model']; ?></h4></td>
                    <td><a class="link-main" href="Samsung/View_Rom.php?Model=<?php echo $row['Model']; ?>" > عرض <span class="fa fa-chevron-right"> </span></a></td>
                  </tr>
                  <?php }    ?>
              </tbody>
            </table>
