...
<html lang="en">
<head>
<title>ROM [FIRMWARE] <?php echo $row['Model_Name']; ?> <?php echo $row['Modell']; ?></title>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta name="description" content="Download all Official Rom for device  <?php echo $row['Model_Name']; ?> , <?php echo $row['Modell']; ?>  all version and all protect and all baseband">
<meta name="keywords" content="download rom samsung <?php echo $row['Model_Name']; ?>,samsung <?php echo $row['Model_Name']; ?> rom download,روم <?php echo $row['Model_Name']; ?>, روم <?php echo $row['Modell']; ?>, روم عربي <?php echo $row['Modell']; ?>, روم معرب <?php echo $row['Modell']; ?>, روم رسمي <?php echo $row['Modell']; ?>, تعريب <?php echo $row['Modell']; ?>,rom <?php echo $row['Modell']; ?>,rom <?php echo $row['Model_Name']; ?>,rom arabic <?php echo $row['Modell']; ?>, full rom <?php echo $row['Modell']; ?>,تعريب <?php echo $row['Model_Name']; ?>">
...
></span> LANGUAGE <span class="caret"></span></a>
            <ul class="dropdown-menu">
              <li class="arabic"><a  href="../../AR/Samsung/View_Rom.php?Model=<?php echo $row['Modell']; ?>">العربية</a></li>
              <li role="separator" class="divider"></li>
              <li><a href="View_Rom.php?Model=<?php echo $row['Modell']; ?>">English</a></li>
            </ul>
          </li>
        </ul>
    ...
    <div class="row">
      <div class="col-xs-12 visible-xs">
        <div class="all-device-mobile">
          <h4 class="wow flash" data-wow-duration="4s" data-wow-iteration="100" >STOCK ROM FOR <?php echo $roms[0]['Model_Name']; ?> - <?php echo $roms[0]['Modell']; ?></h4>
        </div>
        <?php foreach($roms as $row) {?>
          <div class="mobile-table btn-block text-center "> <a href="Download_Rom.php?Oid=<?php echo $row['Oid']; ?>">
            <div class="file-type "> STOCK ROM
              <p> <span class="glyphicon glyphicon glyphicon-eye-open"></span> <?php echo $row['view']; ?></p>
            </div>
            <?php echo $row['Model_Name']; ?> - <?php echo $row['Modell']; ?> - <?php echo $row['Besband']; ?> -   V( <?php echo $row['Vrsion']; ?> ) </a></div>
          <?php } ?>
      </div>
    </div>
    <!-- /.table for mobile-->
            ...
            <tbody>
              <?php foreach($roms as $row){?>
                <tr>
                  ...
                  <td><a class="link-main" href="Download_Rom.php?Oid=<?php echo $row['Oid']; ?>" >VIEW <span class="fa fa-chevron-right"> </span></a></td>
                </tr>
                <?php } ?>
            </tbody>
          </table>
